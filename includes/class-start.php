...
class Dmm_Start {
        ...
        try {
            ...
            if (isset($_POST['dmm_submitted'])) {
                ...
                else
                {
                    ...
                    if (isset($_POST['dmm_recurring_interval']))
                    {
                        ...
                        {
                            case 'one':
                                $interval = __('One-time donation', DMM_TXT_DOMAIN);
                                break;
                            case 'month':
                                $interval = __('Monthly', DMM_TXT_DOMAIN);
                                break;
                            case 'quarter':
                                $interval = __('Each quarter', DMM_TXT_DOMAIN);
                                break;
                            case 'year':
                                $interval = __('Annually', DMM_TXT_DOMAIN);
                                break;
                        }
                    }
            ...
            if (isset($_GET['dmm_id'])) {
                ...
                if ($payment->status == 'paid')
                {
                    ...
                    {
                        wp_redirect(get_option('dmm_redirect_success') != '-1' ? home_url(get_option('dmm_redirect_success')) : get_page_link() . '?dmm_redirect=true&dmm_id=' . $_GET['dmm_id']);
                        exit;
                    }
                ...
                else
                {
                    if (!isset($_GET['dmm_redirect']))
                    {
                        wp_redirect(get_option('dmm_redirect_failure') != '-1' ? home_url(get_option('dmm_redirect_failure')) : get_page_link() . '?dmm_redirect=true&dmm_id=' . $_GET['dmm_id']);
                        exit;
                    }
                ...
                <form action="<?php echo esc_url($_SERVER['REQUEST_URI']);?>" class="<?php echo esc_attr(get_option('dmm_form_cls'));?>" method="post">
                        ...
                        <p>
                            <select id="dmm_interval" name="dmm_recurring_interval" style="width: 100%" class="<?php echo esc_attr(get_option('dmm_fields_cls'));?>" onchange="dmm_recurring_methods(this.value);">
                                ...
                                <?php if (isset($intervals['month'])) { ?>
                                    <option value="month" <?php echo ($selected_interval == 'month' ? 'selected' : '');?>><?php echo esc_html_e('Monthly', DMM_TXT_DOMAIN);?></option>
                                <?php } ?>
                                <?php if (isset($intervals['quarter'])) { ?>
                                    <option value="quarter" <?php echo ($selected_interval == 'quarter' ? 'selected' : '');?>><?php echo esc_html_e('Each quarter', DMM_TXT_DOMAIN);?></option>
                                <?php } ?>
                                <?php if (isset($intervals['year'])) { ?>
                                    <option value="year" <?php echo ($selected_interval == 'year' ? 'selected' : '');?>><?php echo esc_html_e('Annually', DMM_TXT_DOMAIN);?></option>
                                <?php } ?>
                            </select>
                        </p>
                    <?php } else { ?>
                    ...
                    <?php if (isset($dmm_fields['Name']['active']) && $dmm_fields['Name']['active']) { ?>
                        <p <?php echo ($dmm_fields['Name']['active'] ? '' : 'style="display:none"');?>>
                            <?php echo esc_html_e('Name', DMM_TXT_DOMAIN) . (isset($dmm_fields['Name']['required']) && $dmm_fields['Name']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <?php if (isset($dmm_fields['Company name']['active']) && $dmm_fields['Company name']['active']) { ?>
                        <p>
                            <?php echo esc_html_e('Company name', DMM_TXT_DOMAIN) . (isset($dmm_fields['Company name']['required']) && $dmm_fields['Company name']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <?php if (isset($dmm_fields['Email address']['active']) && $dmm_fields['Email address']['active']) { ?>
                        <p <?php echo ($dmm_fields['Email address']['active'] ? '' : 'style="display:none"');?>>
                            <?php echo esc_html_e('Email address', DMM_TXT_DOMAIN) . (isset($dmm_fields['Email address']['required']) && $dmm_fields['Email address']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <?php if (isset($dmm_fields['Phone number']['active']) && $dmm_fields['Phone number']['active']) { ?>
                        <p>
                            <?php echo esc_html_e('Phone number', DMM_TXT_DOMAIN) . (isset($dmm_fields['Phone number']['required']) && $dmm_fields['Phone number']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <?php if (isset($dmm_fields['Address']['active']) && $dmm_fields['Address']['active']) { ?>
                        <p>
                            <?php echo esc_html_e('Address', DMM_TXT_DOMAIN) . (isset($dmm_fields['Address']['required']) && $dmm_fields['Address']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                        ...
                        <p>
                            <?php echo esc_html_e('Zipcode', DMM_TXT_DOMAIN) . (isset($dmm_fields['Address']['required']) && $dmm_fields['Address']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                        ...
                        <p>
                            <?php echo esc_html_e('City', DMM_TXT_DOMAIN) . (isset($dmm_fields['Address']['required']) && $dmm_fields['Address']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                        ...
                        <p>
                            <?php echo esc_html_e('Country', DMM_TXT_DOMAIN) . (isset($dmm_fields['Address']['required']) && $dmm_fields['Address']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <?php if (isset($dmm_fields['Project']['active']) && $dmm_fields['Project']['active']) { ?>
                        <p>
                            <?php echo esc_html_e('Project', DMM_TXT_DOMAIN) . (isset($dmm_fields['Project']['required']) && $dmm_fields['Project']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <?php if (isset($dmm_fields['Message']['active']) && $dmm_fields['Message']['active']) { ?>
                        <p>
                            <?php echo esc_html_e('Message', DMM_TXT_DOMAIN) . (isset($dmm_fields['Message']['required']) && $dmm_fields['Message']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
    ...
    private function dmm_payment_methods($mollie) {
        ...
        if ($option == 'list')
        {
            ...
            {
                $methods .=  '<label ' . $this->dmm_recurring_method($method->id) . '><input type="radio" name="dmm_method" value="' . $method->id . '" ' . ($first ? 'checked' : '') . '> <img style="vertical-align:middle;display:inline-block" src="' . esc_url($method->image->normal) . '"> ' . esc_html__($method->description, DMM_TXT_DOMAIN) . '<br></label>';
                $first = false;
            }
        }
        elseif ($option == 'list_no_icons')
        {
            foreach ($mollie->methods->all() as $method)
            {
                $methods .=  '<label ' . $this->dmm_recurring_method($method->id) . '><input type="radio" name="dmm_method" value="' . $method->id . '" ' . ($first ? 'checked' : '') . '> ' . esc_html__($method->description, DMM_TXT_DOMAIN) . '<br></label>';
                $first = false;
            }
        }
